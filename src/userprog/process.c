...
process_execute (const char *cmdline) 
{
  ...
  file_name = strtok_r(file_name, " ", &save_ptr); //the first word is a file name

  /* Create a new thread to execute FILE_NAME. 
   Create a PCB and pass it as an argument to start_process */
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  int dummy = 0, i;
  for(i=0; i<20 * 10000 * 10000; ++i) dummy += i;
  ASSERT(dummy != 0);
  return -1;
}
