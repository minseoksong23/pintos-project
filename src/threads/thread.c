...
void thread_tick (int64_t tick) 
{
  ...
  /* Wake up any thread whose ticks_end has been expired */
  thread_awake(tick);

  /* Enforce preemption. */
  if (++thread_ticks >= TIME_SLICE)
    intr_yield_on_return (); // Checks if the time slice for the current thread has expired
}
  ...
  for (e = list_begin (&wait_list); e != list_end (&wait_list); e = list_next (e))
    {
      ...
      if (t->sleep_endtick <= current_tick) {
        ...
        t->sleep_endtick = 0; // reset the scheduled awake time
...
thread_sleep_until (int64_t ticks_end)
{
  ...
  list_push_back (&wait_list, &t->waitelem); // thread pushed to the last entry of wait-list
...
thread_unblock (struct thread *t) 
{
  ...
  old_level = intr_disable (); // save the old interrupt level and disable it
...
void
thread_yield (void) // give up processor time if it was running. Even if it wasn't, put it into ready state
{
  struct thread *cur = thread_current ();
  enum intr_level old_level;
  
  ASSERT (!intr_context ()); // shouldn't be on interrupt context

  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list, &cur->elem, priority_less_func_thread, NULL);
  cur->status = THREAD_READY;
  schedule (); // calls the scheduler to select the next thread to run.
  intr_set_level (old_level);
}
...
thread_set_priority (int new_priority) 
{
  ...
  // remember that original_priority is a priority before donation, so we may only change that
  else {
    t_current->original_priority = new_priority;
  }

  // if current thread gets its priority decreased, then yield
  // (foremost entry in ready_list has the highest priority)
...
schedule (void) 
{
  ...
  thread_schedule_tail (prev); // clean up
}
