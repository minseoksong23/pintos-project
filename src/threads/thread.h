...
struct thread
  {
    ...
    struct list_elem allelem;           /* Element as a thread in all threads list. */
    struct list_elem waitelem;          /* Element as a thread in a waited list */
    int original_priority;              /* priority before donation */
    struct lock *waiting_on_lock;       /* lock that the thread is waiting on, if any */
    struct list locks;                  /* list of locks that the thread holds, if any */

    int64_t sleep_endtick;              /* The specific point in time, measured in ticks, when the thread is scheduled to wake up from sleep.  */
